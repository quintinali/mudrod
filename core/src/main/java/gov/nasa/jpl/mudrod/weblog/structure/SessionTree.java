...
public class SessionTree extends MudrodAbstract {
	...
	/**
	 * findLatestRefer: Find parent node whose visiting url is equal to the
	 * refer url of a session node
	 *
	 * @param node:
	 *            {@link SessionNode}
	 * @param refer:
	 ...
	 *             data.
	 */
	public List<RankingTrainData> getRankingTrainData(String indexName, String sessionID)
			throws UnsupportedEncodingException {
	 ...
	 *             data.
	 */
	public List<RecomTrainData> getRecomTrainData(String indexName, String sessionID)
			throws UnsupportedEncodingException {

		List<RecomTrainData> trainDatas = new ArrayList<>();

		List<SessionNode> branches = this.root.getChildren();
		for (int i = 0; i < branches.size(); i++) {
			SessionNode branch = branches.get(i);
			List<SessionNode> branchDataNodes = getNodes(branch, "dataset");
